...
```{r txt_query}
txt.out <- paste("chr", chr, "_", site.range, ".txt", sep="")
...
```{r vcf_query}
vcf.out <- paste("chr", chr, "_", site.range, ".vcf", sep="")
...
beagle.out <- paste("chr", chr, "_", site.range, sep="")
system(paste('cat ', vcf.out, ' | java -jar vcf2beagle.jar ? ', beagle.out, sep=""))
```

I ran BEAGLE on this file for haplotype imputation.

```{r beagle, cache=TRUE}
system(paste('java -Xmx1000m -jar beagle.jar unphased=', paste(beagle.out, ".bgl.gz", sep=""), ' missing=? out=imp', sep=""))
# extract file
system(paste('gunzip ', paste("imp.", beagle.out, ".bgl.gz.phased.gz", sep=""), sep=""))
```

I loaded the imputed haplotypes file into R to determine how many unique haplotypes.

```{r load_hap}
giraffe.haps <- read.table(paste("imp.", beagle.out, ".bgl.gz.phased", sep=""), colClasses="character")
